...
- Save figure. This opens up a dialog allowing you to save the current graph panel state as an image. See the dedicated section on [saving pictures in Overseer](pictures) for more details on saving images. 
...
- Grab as initial. See [the dedicated section on this](save-as-initial) for details. 
- Viewing mode (not pictured). Here you can swap the right-side of Overseer between a plot pane and a logs pane. The logging system is discussed further [here](logger).
- Parameter change response (not pictured). Controls how Overseer responds to changes the user makes in the control panel. See [the section on writing simulations](writing-simulations) for more information on this. Beginners are advised to stay away from the "Send message" option, and stick to either restart or widgets decide (which is the same as restart if the user does not touch the default widget settings when creating them).
- Transparent background toggle. It is recommended to uncheck this before saving a picture, and that's why it is here.
- Animate sim. If for some reason you don't want Overseer to report incremental progress, you can uncheck this. Instead of rendering progress as it comes in, Overseer will wait until receiving a finished signal and then render the whole thing at once. This checkbox **does not effect the efficiency of your simulation in any way*** (see [the discussion of internal operation and efficiency](internal-operation-efficiency)). However, it can save you some system resources. Maybe. On a single core. 
...
For more details on what you can make for the control panel, see the [dedicated page on making control widgets](control-panel-widgets). The focus of this section will be on the graph panel and the control *in relation to it*, i.e. the Plot Controls tab.
(slots)=
