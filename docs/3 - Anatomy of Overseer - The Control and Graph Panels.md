...
You can tell which slot this widget controls via the divider at the top - in this case the slot (1,1), i.e. the top left slot. The top set of settings allows you to precisely tune visible coordinate range of a slot, save a current view for easy recovery later, or set a current view as the default for a category (see the section on [plots and categories](7%20-%20Plots%20and%20Categories.md)). Below that, we have options to change the legend size, where the legend appears, whether the legend appears at all, and whether labels for the x-axis, y-axis or title appear at all. 
