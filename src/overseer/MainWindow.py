...
class MainWindow(qw.QMainWindow):
    ...
    def _stop_sim(self, *, force: bool= False, rerun: bool= False, never_escalate: bool= True):
        """Compatibility wrapper for callers that require a blocking stop."""
        return self._stop_sim_blocking(
            force=force,
            rerun=rerun,
    ...
    def _stop_sim_gracefully(self, rerun= False):
        # Keep the bridge alive so it can receive DONE and finish the teardown.
        self._request_stop(
            rerun=rerun,
            escalation_delay_ms=None,
        )

    def _stop_sim_pragmatically(self, rerun= False):
        # This is the same stop-then-escalate path used by F5/F7 restarts.
        # A second press while STOPPING escalates immediately.
        if self._sim_state == "STOPPING":
            self._escalate_stop_if_needed(self._run_id)
            return

        self._request_stop(
            rerun=rerun,
            escalation_delay_ms=1500,
        )
    ...
    def _kill_sim(self, rerun= False):
        ...
        if ctrl is None or not ctrl.is_alive():
            ...
            if rerun:
                ...
                self.status_bar.showMessage("Simulation is already stopped.", msecs=3000)
            return True
    ...
    def _stop_sim_blocking(
        ...
        *,
        ...
        """Stop synchronously for code paths that must not mutate state until the child exits."""
        if not rerun:
            self._rerun_pending = False
            self._pending_restart_update = None
    ...
    def _halt_sim_stack(
        ...
        *,
        ...
        """Stop/reap the process, then dispose the bridge and queue once it is safe."""
        try:
        ...
        if ctrl is not None:
            try:
                if ctrl.is_alive():
                    ctrl.request_stop(force=force)
                    ctrl.join(timeout=2.0)

                    if ctrl.is_alive() and not never_escalate:
                        self.status_bar.showMessage(
                            "Sim is unresponsive. Escalating...",
                            msecs=3000,
                        )
                        ctrl.request_stop(force=True)
                        ctrl.join(timeout=2.0)
                else:
                    # Reap a process that exited before its DONE message was handled.
                    ctrl.join(timeout=0)

                stopped = not ctrl.is_alive()
            except Exception as e:
                logger.exception("Error while stopping simulation")
                self.status_bar.showMessage(
                    f"Error while stopping simulation: {e}",
                    msecs=5000,
                )
                stopped = False
    ...
    def _schedule_rebuild(self):
        self._curve_visibility_dialog = Curve
    ...
    def _load_prior_results(self, filename):
        if self._sim_state in {"RUNNING", "STOPPING"}:
            if not self._stop_sim_blocking(force=False, never_escalate=False):
    ...
    def _request_stop(
        ...
        *,
        rerun: bool=False,
        escalation_delay_ms: int | None=1500,
    ) -> bool:
        if rerun:
        ...
        try:
            ctrl.request_stop(force=False)
        except Exception as e:
            logger.exception("Error while requesting simulation stop")
            self.status_bar.showMessage(
                f"Error while requesting stop: {e}",
                msecs=5000,
            )
            return False
        ...
        if escalation_delay_ms is not None:
            stopping_run_id = self._run_id
            qc.QTimer.singleShot(
                escalation_delay_ms,
                lambda run_id=stopping_run_id: self._escalate_stop_if_needed(run_id),
            )
    ...
    def _escalate_stop_if_needed(self, run_id: int | None=None):
        # Ignore a timer left over from an older run. Without this guard, an old
        # timer can force-kill a newer run that happens to be stopping too.
    ...
    def _finish_requested_stop(self):
        rerun = bool(self._rerun_pending)
        self._rerun_pending = False
        self._sim_state = "IDLE"
        ...
        if rerun:
            ...
            self.status_bar.showMessage("Sim stopped successfully.", msecs=3000)
    ...
    def _on_sim_done(self):
        sender = self.sender()
        sender_run_id = getattr(sender, "run_id", self._run_id)
        if sender_run_id != self._run_id:
            return

        self._anim_timer.stop()
        was_stopping = self._sim_state == "STOPPING"

        if not was_stopping and self._pending_traj is not None:
            self.show_partial_results(self._pending_traj, self._pending_t)
            self.traj, self.t = self._pending_traj, self._pending_t

        stopped = self._halt_sim_stack(
            force=False,
            clear_pending=was_stopping,
            clear_queue=True,
            never_escalate=True,
        )
    ...
    def load_demo(self, demo_name, autostart= True):
        ...
        self.toolbar.graph_panel = new_graph_panel
